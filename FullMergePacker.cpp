...
void FullMergePacker::pack(const DataViewer* dv, const vector<unsigned>& indices, vector<Cluster>& clusters) const
{
	if(indices.size() == 0)
		return;
...
bool FullMergePacker::fullMergeClusters(const DataViewer *D, vector<Cluster>& clusters) const
{
...
float FullMergePacker::clusterDistance(const DataViewer* D, const Cluster& a, const Cluster& b) const
{
